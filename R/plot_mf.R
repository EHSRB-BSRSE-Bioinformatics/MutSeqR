...
#' @param mf_data A data frame containing the mutation frequency data. This is obtained
#' from the calculate_mut_freq function with SUMMARY = TRUE.
#' @param sample_col The name of the column containing the sample names.
#' @param mf_type The type of mutation frequency to plot. Options are "unique", "clonal",
#' "both", or "stacked". If "both", the unique and clonal mutation frequencies are plotted
#' side by side. If "stacked", the difference between the unique and clonal MF is stacked
#' on top of the unique MF such that the total height of both bars represent the clonal MF.
...
#' Count labels display the number of mutations, MF labels display the mutation frequency.
...
if (mf_type %in% c("unique", "clonal")) {
    ...
    if (mf_type == "stacked") {
        plot_data <- transform(plot_data, mf_clonal = mf_clonal - mf_unique)
...
if (is.null(custom_palette)) {
    ...
    gradient <- colorRampPalette(colors = c("#c5e5fc", "#5ab2ee", "#12587b", "#263247",
                                          "#b23946", "#ff5264", "#ffb9c1", "#ffedef"))
    palette <- gradient(n_colors)
    } else {
    palette <- custom_palette
}
   
plot <- ggplot(plot_data, aes(x = sample_col, y = mf_col, fill = factor(fill))) +
    geom_bar(stat = "identity", position = position, color = "darkgrey") +
    geom_text(aes(y = mf_col, label = label), position = label_position, vjust = -0.5, size = 2, color = "black") +
    yscale +
    labs(title = title,
        fill = fill_label) +
    ylab(y_lab) +
