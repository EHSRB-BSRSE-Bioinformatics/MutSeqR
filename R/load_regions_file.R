...
load_regions_file <- function(regions, 
  ...
  if (regions == "human") {
    regions_df <- read.table(system.file("extdata", "genic_regions_hg38.txt", package = "DupSeqR"), header = TRUE)
  } else if (regions == "mouse") {
    regions_df <- read.table(system.file("extdata", "genic_regions_mm10.txt", package = "DupSeqR"), header = TRUE)
    ...
    warning("Invalid regions parameter. Choose from 'human', 'mouse', or 'custom'.")
  }
  return(regions_df)
}