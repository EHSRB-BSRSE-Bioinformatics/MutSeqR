---
title: "Duplex Sequencing Signatures - PRC"
...
```{r import_data}
#Aggregate data, not meaningful, did not use
# Import trinucleotide frequencies from spreadsheet saved as TSV
#mutation.df <- read.table("~/GitHub/Duplex-Sequencing/per mouse/PRC_BM_Trinuc_Freqs_bydose.txt", header=T, sep="\t")
...
#rownames(tax) <- tax
#tax = tax_table(tax)
#sampledata <- data.frame(dose=factor(c("0", "6.25", "12.5", "25")), chemical="PRC") # Need a dummy column of chemical...
#row.names(sampledata) <- c("0", "6.25", "12.5", "25")
...
#dist_methods <- dist_methods[!dist_methods %in% c("cao","ANY","morisita")]
#print(dist_methods)
...
###################################################
#Load Data
#Add Dose and location_relative_to_genes
genic_regions <- read.delim("~/GitHub/Duplex-Sequencing/per mouse/input files/genic_regions.txt")
...
per.nucleotide.data.all <- read.delim("~/GitHub/Duplex-Sequencing/per mouse/input files/prj00125_mouse_bonemarrow_allmuts_withtrinucleotidecontext_filtered.mut", header=T, sep="\t")
...
colnames(per.nucleotide.data)[6:8] <- c("contig","start","end")

per.nucleotide.data[1:10,]

#Note: col names
#mut_depth = final_somatic_alt_depth
#total_depth_ = informative_total_depth

#Create a new column for unique mutation counts (if you want to run the analysis without clonal expansion)
#per.nucleotide.data.unique <- per.nucleotide.data %>%
#  mutate(unique_mut_depth = ifelse(!variation_type=="no_variant", 1, 0))
...
trinucleotide_frequencies_depth <- per.nucleotide.data %>%
  ...
  mutate(group_depth = sum(total_depth)) %>%
  ungroup() %>%
  group_by(context_with_mutation, sample) %>%
  mutate(frequency = (sum(mut_depth)/group_depth) ) %>%
  ungroup() %>%
