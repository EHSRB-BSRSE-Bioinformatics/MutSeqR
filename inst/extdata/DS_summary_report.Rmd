...
Corrects the total_depth at overlapping positions within a single sample.
Overlapping positions are defined as those sharing the same sample, contig, and
start values. For these positions, the total_depth will be retained for only one row.
All other rows will have their total_edpth set to 0 to prevent double-counting. This
parameter is set to TRUE automatically when overlapping positions are detected within
the mutation data.
...
plot3.2 <- plot_mf(mf_data = plot_data,
                 group_col = "sample",
                 plot_type = "bar",
...
if (params$bmd_method == "proast") {
  bmd <- bmd_proast(mf_data = mf_data,
                    ...
                    num_bootstraps = 2, # parameterize this. 50/200
                    plot_results = TRUE,
                    output_path = NULL)
...
```{r plot-spectra-clustered, fig.cap=caption}
