...
params:
  ...
  regions: 
    ...
    input: select
    value: "TSpanel_mouse"
    choices: ["TSpanel_mouse", "TSpanel_human", "TSpanel_rat", "custom_interval", "none"]
  custom_regions_file:
...
MF_per_sample <- calculate_mf(mutation_data = mutation_data,
...
plot_min_mf <- plot_mf(mf_data = MF_per_sample,
                          sample_col = "sample",
                          mf_type = "min",
...
plot_min_mf
# Plot both max and min MF, stacked
plot_max_mf <- plot_mf(mf_data = MF_per_sample,
                          sample_col = "sample",
                          mf_type = "stacked",
                          fill_col = params$dose_column,
                          sample_order = "arranged",
...
plot_model <- ggplot2::ggplot(plot_data, aes(x = dose, y = Estimate.x)) +
  ...
  labs(title = "Min Mutation Frequency by Dose",
...
ma$BMD
ma$model_plots$sample_MF_min
ma$BMD_plot
}
# MF per target Analysis
MF_per_target <- calculate_mf(data = mutation_data,
...
MF_per_sample <- calculate_mf(data = mutation_data,
                                    cols_to_group = c("sample"),
                                    subtype_resolution = "none",
...
plot_model <- ggplot2::ggplot(plot_data, aes(x = dose, y = Estimate.point, fill = sampling_time)) +
  ...
  labs(title = "Minimum Mutation Frequency by Dose",
       y = "Mutation Frequency (mutations/bp)",
       x = x_lab) +
...
trinuc_spectra <- calculate_mf(data = mutation_data,
...
if ("dose" %in% colnames(mutation_data)) {
  ...
  simple_spectra <- calculate_mf(data = mutation_data,
                                      cols_to_group = "sample",
...
plot_data <- simple_spectra %>%
  ...
  summarize(avg_MF = mean(sample_MF_min),
            std_error = sd(sample_MF_min)/sqrt(n())) %>%
  ungroup()
...
ggplot(six_base_summary_group, aes(x = normalized_subtype, y = proportion_min, fill = factor(dose))) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.7) +
...
ggplot(six_base_summary_group, aes(x = normalized_subtype, y = dose_MF_min, fill = factor(dose))) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.7) +
...
label_positions <- data.frame(
  ...
  y_position = max(plotdata$dose_MF_min) * 1.1  # Adjust the multiplier based on your desired position above the max y-value
)

# Calculate upper limit for y-axis
y_max_limit <- max(plotdata$dose_MF_min) * 1.5  # Adjust the multiplier based on your desired position above the max y-value

# Create the bar plot with facets arranged vertically, increased x-axis spacing, and color mapping
ggplot(plotdata, aes(x = reorder(normalized_context_with_mutation, -as.numeric(factor(mutation_type))), y = dose_MF_min, fill = factor(mutation_type))) +
