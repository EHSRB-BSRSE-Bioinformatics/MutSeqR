...
#mut_file <- ../../data/Jonatan_Mutations_in_blood_and_sperm_samples_221021_MM.txt
...
project <- "Mouse Test"
project_genome = "mm10"
mut_file <- "../../data/BaP_BM_C4.1.consensus.variant-calls.genome.mut"
regions_file <- "../../inst/extdata/genic_regions_mm10.txt"
  
mutation_data <-
  import_mut_data(mut_file = file.path(mut_file),
                  regions_file = file.path(regions_file),
                  rsids = T) #%>%
...
six_base_summary <- calculate_mut_freq(data = as.data.frame(mutation_data), subtype_resolution = "6base", summary = T)
...
as.data.frame(mutation_data) %>% dplyr::group_by(ref) %>% tally()
as.data.frame(mutation_data) %>% dplyr::group_by(short_ref) %>% tally()
as.data.frame(mutation_data) %>% dplyr::group_by(normalized_ref) %>% tally()
...
recurring_mutations <- mutation_data_cpgs %>% dplyr::group_by(seqnames,start,end) %>% tally() %>% dplyr::arrange(-n)

recurring_mutations_group <- mutation_data_cpgs %>% dplyr::group_by(seqnames,start,end,sample) %>% tally() %>% dplyr::arrange(-n)


recurring_mutations_rsid <- mutation_data_cpgs %>% dplyr::group_by(id) %>% tally() %>% dplyr::arrange(-n)
```
