...
## Benchmark Dose Analysis {.tabset}
...
### BMD Confidence Interval Plot

```{r bmd-plot-caption, eval=params$bmd, include=FALSE}
caption <- paste(
...
```{r bmd-plot, eval=params$bmd, fig.cap=caption, fig.width=8, fig.height=6}
...
if (is.null(params$exp_variable)) { # some how make these plots tabs
  data <- mf96
  group_col <- "sample"
} else {
  data <- mf96_expv
...
# Recurring mutations {.tabset}

## Recurring Mutations Whole Data

```{r, recurrent-mutations}

recurring_mutations <- filtered_mutation_data %>%
...
no_recur <- nrow(recurring_mutations)
no_recur_sum <- sum(recurring_mutations_sample$n)
```

Within the entire dataset, `r no_recur` mutations were found to occur in more than one sample.
...
```{r lolliplot-recur, eval=params$do_regions_analysis, fig.cap=caption, fig.width=12, fig.height=6}
...
for (contig_name in unique_contigs) {
  ...
  plot_list[[as.character(contig_name)]] <- p
}
plot_list
```

## Recurring Mutations Within Samples: Mutations with >1 alternate allele
...
## Recurring Mutations Within `r params$exp_variable`

```{recur-per-expv, eval=!is.null(params$exp_variable)}
...
DT::datatable(recur_mut_expv_summary,
  caption = caption)
...
```{r recurrent-rsids, eval= "id" %in% colnames(filtered_mutation_data)}
recurring_mutations_rsid <- filtered_mutation_data %>%
  dplyr::filter(!variation_type %in% "no_variant") %>%
  dplyr::group_by(dplyr::id) %>%
  dplyr::tally() %>%
  dplyr::arrange(-n)
```
