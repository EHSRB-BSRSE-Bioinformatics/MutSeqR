...
params:
  ...
  genome: "mm10"
  mut_data: "../../Test Data/prj00125_PRC_BM_variany-calls.genome.mut"
...
sample_MF_summary <- calculate_mut_freq(data = mutation_data,
                                       cols_to_group = c("sample"),
                                       subtype_resolution = "none",
                                       vaf_cutoff = params$vaf_cutoff,
                                       summary = T) 

sample_regions_MF_summary <- calculate_mut_freq(data = mutation_data,
...
ggplot(sample_MF_summary, aes(x = sample, y = sample_MF_unique, fill = dose)) +
...
ggplot(sample_MF_summary, aes(x = sample, y = sample_MF_clonal, fill = dose)) +
  geom_bar(stat = "identity", alpha = 0.7) +
...
# Average MF by dose for both clonal and unique ## NEED TO ADJUST FOR Y AXIS SCALE
plot_data <- sample_MF_summary %>%
  group_by(dose) %>%
...
plot_data <- sample_regions_MF_summary %>%
