...
``` {r set-plot-size3.2, include=FALSE, eval = !no_mf}
fig_width <- 8
fig_height <- 6
labels <- "count"
...
```{r plot-mf, fig.width=fig_width, fig.height=fig_height, fig.cap=caption, eval = !no_mf}
plot_data <- mf_data
if (!is.null(params$exp_variable)) {
...
# Skip this if using a precalc depth because it will add the exp_variable.
if (!is.null(params$exp_variable) & is.null(params$precalc_depth_data_base6)) {
...
if (!is.null(params$precalc_depth_data_base96)) {
  precalc_depth_data_base96 <- read.delim(
    ...
    precalc_depth_data_base96 <- precalc_depth_data_base96 %>%
