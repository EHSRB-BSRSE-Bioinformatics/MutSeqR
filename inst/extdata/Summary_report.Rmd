...
```{r check-for-suggests, include=FALSE}
check_pkgs <- function(pkgs) {
  ...
  if (length(missing_packages) > 0) {
    stop(paste0(
      "Rendering halted: The following required packages are not installed:\n  ",
...
plotting <- c(
  "colorspace", # plot_mean_mf
  "dendsort", # plot_spectra 
  "ggh4x", # plot_spectra
  "ggrepel", # plot_mf plot_mean_mf
  "gtools", # plot_mf plot_mean_mf plot_spectra
  "packcircles", #plot_bubbles
  "patchwork", # plot_spectra
  "RColorBrewer" # plot_mf plot_mean_mf plot_spectra plot_bubbles plot_lollipop
)
stats <- c("doBy", "car", "lme4") # model_mf
general <- c("openxlsx", "BSgenome")
pkg_ls <- c(plotting, general)
if (!is.null(params$exp_variable)) {
pkg_ls <- c(pkg_ls, stats)
}
...
if (!no_variants_present) {
  cat("WARNING! The imported mutation data contains a total_depth column",
    ...
    "(see precalc_depth_data) if it is not possible to import all sequenced",
    "sites into R."
  )
}
```
...
```{r plot3.2-caption, include=FALSE}
...
```{r plot-mf, fig.width=fig_width, fig.height=fig_height, fig.cap=caption}
plot_data <- mf_data
if (!is.null(params$exp_variable)) {
...
``` {r set-plot-size3.3, eval=!is.null(params$exp_variable), include=FALSE}
fig_width <- 8
fig_height <- 6
...
```{r plot3.3-caption, eval=!is.null(params$exp_variable), include=FALSE}
# Define your caption components
caption <- paste0(
  "Mean Mutation Frequency (MF) Minimum",
  if (plot_max) " and Maximum",
  " (mutations/bp) per ", params$exp_variable, ". Lines are the mean ± S.E.M.",
...
```{r plot-mean-mf, eval=!is.null(params$exp_variable), fig.width=fig_width, fig.height=fig_height, fig.cap=caption}
...
```{r glm-hist-caption, eval=!is.null(params$exp_variable), include=FALSE}
caption <- paste(
...
```{r glm-residuals-hist, eval=!is.null(params$exp_variable), fig.cap=caption}
...
```{r glm-qq-caption, eval=!is.null(params$exp_variable), include=FALSE}
caption <- paste0(
  "GLM residuals of MFmin modelled as an effect of ",
...
```{r glm-residuals-qq, eval=!is.null(params$exp_variable), fig.cap=caption}
...
```{r plot4.16-caption, eval=!is.null(params$exp_variable), include=FALSE}
caption <- paste0(
...
```{r plot-glm, eval=!is.null(params$exp_variable), fig.cap=caption}
...
```{r bmd-analysis, eval=params$bmd, results='hide', fig.show = 'hide'}
if (plot_max) {
...
```{r bmd-exphill-tab, eval=params$bmd&params$bmd_method=="proast", results='asis'}
cat("#### Exponential & Hill Models\n\n")
```

```{r bmd-exphill-cap, eval=params$bmd&params$bmd_method == "proast", include=FALSE}
caption1 <- paste(
...
```{r bmd-exphill-plot1, eval=params$bmd&params$bmd_method == "proast", fig.width=12, fig.height=6, fig.cap=caption1}
replayPlot(bmd$mf_min_Expon_HILL)
```

```{r bmd-exphill-plot2, eval=params$bmd&params$bmd_method == "proast"&plot_max, fig.width=12, fig.height=6, fig.cap=caption2}
replayPlot(bmd$mf_max_Expon_HILL)
```

```{r bmd-invln-tab, eval=params$bmd&params$bmd_method=="proast", results='asis'}
cat("####  Inverse Exponential & Log-Normal Models\n\n")
```

```{r bmd-invln-cap, eval=params$bmd&params$bmd_method == "proast", include=FALSE}
caption1 <- paste(
...
```{r bmd-invln-plot1, eval=params$bmd&params$bmd_method == "proast", fig.width=12, fig.height=6, fig.cap=caption1}
replayPlot(bmd$mf_min_InvExp_LN)
```

```{r bmd-invln-plot2, eval=params$bmd&params$bmd_method == "proast"&plot_max, fig.width=12, fig.height=6, fig.cap=caption2}
replayPlot(bmd$mf_max_InvExp_LN)
```

```{r bmd-boot-tab, eval=params$bmd&params$bmd_method=="proast", results='asis'}
cat("####  Bootstrap Curves\n\n")
```

```{r bmd-boot-cap, eval=params$bmd&params$bmd_method == "proast", include=FALSE}
caption1 <- paste(
...
```{r bmd-boot-plot1, eval=params$bmd&params$bmd_method == "proast", fig.cap=caption1}
replayPlot(bmd$mf_min_boostrap_curves)
```

```{r bmd-boot-plot2, eval=params$bmd&params$bmd_method == "proast"&plot_max, fig.cap=caption2}
replayPlot(bmd$mf_max_boostrap_curves)
```

```{r bmd-ma-tab, eval=params$bmd&params$bmd_method=="toxicr", results='asis'}
cat("####  Model Averaged Model\n\n")
```

```{r bmd-ma-cap, eval=params$bmd&params$bmd_method == "toxicr", include=FALSE}
caption1 <- paste(
...
```{r bmd-ma-plot1, eval=params$bmd&params$bmd_method=="toxicr", fig.cap=caption1}
bmd$mf_min_ma
```

```{r bmd-ma-plot2, eval=params$bmd&params$bmd_method=="toxicr"&plot_max, fig.cap=caption2}
bmd$mf_max_ma
```

#### Cleveland Plots

```{r bmd-cvd-cap, eval=params$bmd, include=FALSE}
caption1 <- paste(
...
```{r bmd-cvd-plot1, eval=params$bmd, fig.cap=caption1}
bmd$mf_min_cleveland
```

```{r bmd-cvd-plot2, eval=params$bmd&plot_max, fig.cap=caption2}
bmd$mf_max_cleveland
```

#### Confidence Interval Plot

```{r bmd-ci-cap, eval=params$bmd, include=FALSE}
caption <- paste(
...
```{r bmd-ci-plot, eval=params$bmd, fig.cap=caption, fig.width=8, fig.height=6}
...
caption <- paste0(
  ...
  if (!no_depth) " normalized to sequencing depth",
  ". Samples are clustered based on the Euclidean distance between their",
  " subtype proportions."
)
```

```{r plot-spectra-clustered, fig.cap=caption, fig.width=12, fig.height=6}
...
```{r plot-spectra-exp-caption, eval=!is.null(params$exp_variable),  include=FALSE}
caption <- paste0(
...
caption <- paste0(
  ...
  if (!no_depth) " normalized to the sequencing depth. ",
...
```{r signature-analysis, eval=params$run_sigfitting}
...
```{r glmm-hist-caption, eval=params$do_regions_analysis, include=FALSE}
caption <- paste0(
...
```{r glmm-residuals-hist, eval=params$do_regions_analysis, fig.cap=caption}
...
```{r glmm-qq-caption, eval=params$do_regions_analysis, include=FALSE}
caption <- paste0(
  "GLMM residuals of MFmin modelled as an effect of ",
  params$region_col,
  if (!is.null(params$exp_variable)) " and ", params$exp_variable,
...
```{r glmm-residuals-qq, eval=params$do_regions_analysis, fig.cap=caption}
...
``` {r glmm-point-estimates, eval=params$do_regions_analysis}
DT::datatable(
...
```{r set-plot-size6.7, include=FALSE}
n_rg <- nrow(rg_levels)
fig_width <- 8
fig_height <- 6
```

```{r plot6.7-caption, eval=params$do_regions_analysis, include=FALSE}
caption <- paste0(
  "Mean Mutation Frequency Minimum (mutations/bp) per ",
  params$region_col,
  if (!is.null(params$exp_variable)) " and ", params$exp_variable,
...
```{r plot-glmm, eval=params$do_regions_analysis, fig.width=fig_width, fig.height=fig_height, fig.cap=caption}
