...
# Check the Mutation Data

```{r check-mut-data}
no_depth <- !dplyr::any(c("depth", "total_depth") %in% colnames(params$mutation_file))
no_variants <-  "no_variants" %in% unique(params$mutation_file$varition_type)
...
Imported `r entries` rows from mutation data.
...
**Correcting the total_depth for overlapping positions: `r !no_depth`.**
...
mf_data <- calculate_mf(
  ...
  d_sep = params$d_sep,
  correct_depth = !no_depth,
  retain_metadata_cols = params$exp_variable
)
no_mf <- !("mf_min" %in% colnames(mf_data))
