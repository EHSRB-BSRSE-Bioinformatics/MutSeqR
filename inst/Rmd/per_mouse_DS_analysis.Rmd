...
cpg_sites <- get_CpG_mutations(get_region_seqs("human"), mutation_data)
# TODO instead of manually specifying human or mouse, it should be a config parameter.

# Mutations at CpG sites...
as.data.frame(cpg_sites) %>% dplyr::group_by(subtype) %>% tally()
...
mutation_data_proportions <- convert_muts_to_proportions(mutation_data, group_var)
...
#tnp_all <- convert_muts_to_proportions(x)
...
```{r combine_results2}
tnp_intergenic <- convert_muts_to_proportions(x)
...
```{r combine_results3}
tnp_genic <- convert_muts_to_proportions(x)
