...
test_that("model_mf works as expected ", {
  ...
  expect_equal(class(model_results$model_data$dose), "factor", info = "Check if the dose column is converted to factor")
  expect_equal(levels(model_results$model_data$dose), c("25", "0", "50", "100"), info = "Check if the reference level for 'dose' is correctly set")
  expect_true(all(c("model_data", "model_formula", "summary",
                   "residuals_histogram",
                   "residuals_qq_plot",
                   "point_estimates_matrix",
                   "point_estimates")
                 %in% names(model_results)), info = "All objects are created on model_results")
  })
