...
Bioconductor reviewers. Major changes include parameter validation and
vectoriation of functions. filter_mut() “snv_in_germ_mnv” parameter was
...
removing all snvs that overlap with germline mnvs). plot_lollipop() can
now colour the plot by different subtype resolutions (previously just
base_6). Fixed plot_spectra() axes labels after clustering (previously
not showing labels).
